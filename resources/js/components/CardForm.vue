<template>
  <form id="check-cc" class="px-8 pt-6 pb-8" action="#cards" @submit.prevent="submitCard">
        ...
        <div class="relative">
          ...
          >cvc
            <svg class="icon icon-sm icon-question hover:text-blue fill-current">
              <use xlink:href="storage/images/symbol-defs.svg#icon-question"></use>
            </svg>
          </label
          >
          <input
            ...
            class="border-b appearance-none text-grey-dark bg-white py-2 inline pl-2"
            maxLength="3"
            size="1"
          />
        </div>
    ...
    <div class="flex item-center justify-center mt-8">
      <button
        :disabled="isSubmit"
        class="w-2/3 shadow bg-blue hover:bg-blue-light focus:shadow-outline font-bold text-white px-2 py-3 rounded"
      >{{ buttonLabel }}
      </button>
    </div>
  </form>
...
<script>
  ...
  const verifyCard = _.debounce(async (cardNumber, callback) =>{
    try {
      console.log(cardNumber);
      let cardinfo = await axios.get(`verify?card_number=${cardNumber}`);
